      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
        <Table>
          <TableHeader className="bg-slate-50/50">
            <TableRow>
              <TableHead className="w-[80px]">{t('col_preview')}</TableHead>
              ...
              <TableRow key={item.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <TableCell>
                  <div className="flex flex-wrap gap-1">
                    {item.requiresFlashing && <span className="bg-amber-50 text-amber-600 border border-amber-100 px-1.5 py-0.5 rounded text-[10px] font-bold">{t('badge_flash')}</span>}
                    {item.requiresTraceability && <span className="bg-indigo-50 text-indigo-600 border border-indigo-100 px-1.5 py-0.5 rounded text-[10px] font-bold">{t('badge_trace')}</span>}
                    {item.isPurchasable && <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-1.5 py-0.5 rounded text-[10px] font-bold">{t('badge_purchased')}</span>}
                  </div>
                </TableCell>
                <TableCell>
      ...
      <Dialog open={!!labelPreviewItem} onOpenChange={(open) => !open && setLabelPreviewItem(null)}>
        <DialogContent className="sm:max-w-xs p-6 flex flex-col items-center gap-4">
          <div className="border-2 border-slate-900 p-4 rounded-lg w-full bg-white flex flex-col items-center">
            ...
            <h3 className="text-sm font-bold text-slate-700 mb-4">{labelPreviewItem?.itemName}</h3>
