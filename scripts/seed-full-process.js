// require('dotenv').config(); // Disabled to avoid dependency on missing .env
const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

// Use environment variable if present, otherwise exit
...
async function main() {
  console.log('🚀 Starting Full Lifecycle Seeding...');

  // 1. Ensure a few items exist
  const items = [
  ...
  console.log('✅ Items created/updated');
  ...
  for (let i = 0; i < 5; i++) {
    const so = await prisma.salesOrder.upsert({
      ...
      update: {
        ...
        orderedQty: 100 + i * 50,
        unitPrice: 299.00,
        status: i === 0 ? 'CLOSED' : (i === 4 ? 'CONFIRMED' : 'CONFIRMED'),
  ...
  // 3. Create Work Orders for the last 4 SOs
  ...
  for (let i = 0; i < 5; i++) {
    ...
    // Create a dummy operation for reporting
    ...
    // Create production reports for the completed/in-progress ones
    if (i <= 2) {
  ...
  console.log('✅ Work Orders and Production Reports created');

  // 4. Create some active Andon issues
  ...
  console.log('✅ 3 Andon Issues created');

  // 5. Create some inventory shortages
  // Warehouse first
